...
**General settings** sets the background and text color for every button. With no background, buttons are gray (`#e5e5e5`) and the text is dark (`#333333`). A button background overrides the general one. The same values are stored in the system settings `managerbuttons.background` and `managerbuttons.color`.
